...
def run_game():
    ...
    try:
        ...
        pyfetch = None  # Для локального запуска
    ...
    async def firebase_register(email, password):
    ...
    async def firebase_login(email, password):
    ...
    class LoginScreen:
        ...
        async def submit(self):
            ...
            if self.mode == "register":
                resp = await self.firebase_register(self.email, self.password)
                if "error" in resp:
                ...
                resp = await self.firebase_login(self.email, self.password)
                if "error" in resp:
                    self.error = resp["error"].get("message", str(resp["error"])) if isinstance(resp["error"], dict) else str(resp["error"])
                elif "idToken" in resp:
                    self.success = True
    ...
    class MatchmakingScreen:
        ...
        def start_invite_checker(self):
            self._invite_task = asyncio.create_task(self.invite_checker())

        async def invite_checker(self):
            while not self.done:
                url = f"{self.firebase_url}/invites/{self.user_login}.json"
                if pyfetch:
                    try:
                    ...
                    for event in pygame.event.get():
                        ...
                        if event.type == pygame.MOUSEBUTTONDOWN:
                            ...
                            if 40 <= x <= 360 and 80 <= y <= 130:
                                ...
                                self.draw()
                                self.stop_invite_checker()
                                await self.find_opponent_queue()
                                return
                        ...
                        if event.type == pygame.KEYDOWN and self.active_field:
                            if event.key == pygame.K_RETURN:
                                if self.mode == 'friend' and self.friend_login:
                                    url = f"{self.firebase_url}/invites/{self.friend_login}.json"
                                    if pyfetch:
                                        await pyfetch(url, method="PUT", headers={"Content-Type": "application/json"}, body=json.dumps({"from": self.user_login, "status": "pending"}))
                                    else:
                                        import requests
                                        requests.put(url, json={"from": self.user_login, "status": "pending"})
                                    self.stop_invite_checker()
                                    wait_screen = InviteWaitScreen(self.screen, self.user_login, self.friend_login, self.firebase_url)
                                    await wait_screen.run()
        ...
        async def find_opponent_queue(self):
            # Добавляем себя в очередь
            url = f"{self.firebase_url}/queue/{self.user_login}.json"
            if pyfetch:
                await pyfetch(url, method="PUT", headers={"Content-Type": "application/json"}, body=json.dumps({"login": self.user_login, "status": "waiting"}))
            else:
                import requests
                requests.put(url, json={"login": self.user_login, "status": "waiting"})
            self.status = "Ожидание соперника..."
            self.draw()
            found = False
            start_time = time.time()
            while not found:
                url_queue = f"{self.firebase_url}/queue.json"
                if pyfetch:
                    resp = await pyfetch(url_queue)
                    queue = await resp.json()
                else:
                    import requests
                    resp = requests.get(url_queue)
                    queue = resp.json() if resp.status_code == 200 else None
                if queue:
                    for login, data in queue.items():
                        if login != self.user_login and data.get("status") == "waiting":
                            ...
                            url1 = f"{self.firebase_url}/queue/{self.user_login}.json"
                            url2 = f"{self.firebase_url}/queue/{login}.json"
                            if pyfetch:
                                await pyfetch(url1, method="DELETE")
                                await pyfetch(url2, method="DELETE")
                            else:
                                import requests
                        ...
                        for event in pygame.event.get():
                            ...
                            if event.type == pygame.MOUSEBUTTONDOWN:
                                ...
                                if 40 <= x <= 160 and 180 <= y <= 230:
                                    url_patch = f"{self.firebase_url}/invites/{self.user_login}.json"
                                    if pyfetch:
                                        await pyfetch(url_patch, method="PATCH", headers={"Content-Type": "application/json"}, body=json.dumps({"status": "accepted"}))
                                    else:
                                        import requests
                                        requests.patch(url_patch, json={"status": "accepted"})
                                    ...
                                    url_patch = f"{self.firebase_url}/invites/{self.user_login}.json"
                                    if pyfetch:
                                        await pyfetch(url_patch, method="PATCH", headers={"Content-Type": "application/json"}, body=json.dumps({"status": "declined"}))
                                    else:
                                        import requests
                                        requests.patch(url_patch, json={"status": "declined"})
    ...
    class OnlineGameSession:
        ...
        async def create_session(self, initial_state):
            url = f"{self.firebase_url}/games/{self.game_id}.json"
            if pyfetch:
                await pyfetch(url, method="PUT", headers={"Content-Type": "application/json"}, body=json.dumps(initial_state))
            else:
                import requests
                requests.put(url, json=initial_state)

        async def update_state(self, state):
            url = f"{self.firebase_url}/games/{self.game_id}.json"
            if pyfetch:
                await pyfetch(url, method="PATCH", headers={"Content-Type": "application/json"}, body=json.dumps(state))
            else:
                import requests
                requests.patch(url, json=state)
    ...
    async def async_main():
        try:
            print("=== PYGBAG MAIN STARTED === (async_main)")
            pygame.init()
            screen = pygame.display.set_mode((720, 720))
...
try:
    ...
    print(f"[PYGBAG TOPLEVEL ERROR] {e}")
    import traceback
    traceback.print_exc()
